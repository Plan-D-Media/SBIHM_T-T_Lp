...
// Uses FormSubmit API to send backend-less email notifications
async function sendEmailNotification(formDataObj, formName) {
  const targetEmail = 'strategy@pland.in';
  
  const payload = {
    _subject: `New Travel & Tourism Management Lead: ${formName} - SBIHM`,
    _cc: 'admissions.sbihm@gmail.com',
    _captcha: 'false', // Disable captcha for seamless AJAX flow
    _template: 'table', // Nicer table format in the email
    ...formDataObj
  };
  ...
  try {
    ...
    if (!response.ok) throw new Error('Email API responded with an error');
    return await response.json();
  } catch (error) {
    console.error('[SBIHM] Email submission failed:', error);
    throw error;
  }
}
...
(function initLeadForm() {
  ...
  form.addEventListener('submit', async e => {
    ...
    try {
      /* Send email notification via FormSubmit API */
      ...
      alert("Application Received!\nOur counsellor will reach out to you within 5 minutes. check your WhatsApp for updates.");
      window.location.href = "thankyou.html";

    } catch (err) {
      setButtonLoading(false);
      showError('Something went wrong. Please try again or WhatsApp us directly at +91 98300 12536.');
    }
  });
})();
...
(function initHeroForm() {
  ...
  form.addEventListener('submit', async e => {
    ...
    try {
      ...
      submitBtn.classList.remove('hero-form__submit--loading');
      submitBtn.disabled = false;
      alert('Something went wrong. Please try again or WhatsApp us at +91 98300 12536.');
    }
  });
...
function openBrochureModal() {
  ...
  pushEvent('brochure_download_modal_open', { course: 'Travel & Tourism Management' });
}
...
(function initBrochureModal() {
  ...
  form.addEventListener('submit', async e => {
    ...
    pushEvent('brochure_download', { course: 'Travel & Tourism Management' });
    ...
    link.download = 'SBIHM-Travel-Tourism-Management-Brochure-2026-27.pdf';
    link.click();
...
(function initTracking() {
  document.addEventListener('click', e => {
    ...
    switch (track) {
      case 'phone_click':
        pushEvent('phone_click', { course: 'Travel & Tourism Management' });
        break;
      case 'whatsapp_click':
        pushEvent('whatsapp_click', { course: 'Travel & Tourism Management' });
        break;
